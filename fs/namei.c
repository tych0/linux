...
/**
 * __generic_permission -  check for access rights on a Posix-like filesystem
 ...
 */
static int __generic_permission(struct user_namespace *user_ns, struct inode *inode, int mask)
{
	int ret;
	...
	if (S_ISDIR(inode->i_mode)) {
		...
		if (!(mask & MAY_WRITE))
			if (ns_capable_wrt_inode_uidgid(user_ns, inode,
							CAP_DAC_READ_SEARCH))
				return 0;
		if (ns_capable_wrt_inode_uidgid(user_ns, inode, CAP_DAC_OVERRIDE))
			return 0;
		return -EACCES;
	}
...
{
	return __generic_permission(&init_user_ns, inode, mask);
}
...
static inline int do_inode_permission(struct user_namespace *user_ns, struct inode *inode, int mask)
{
	...
	return __generic_permission(user_ns, inode, mask);
}
...
/**
 * ns_inode_permission - Check for access rights to a given inode as seen from
 ...
 */
int ns_inode_permission(struct user_namespace *user_ns, struct inode *inode, int mask)
{
	int retval;
...
EXPORT_SYMBOL(ns_inode_permission);
...
{
	return ns_inode_permission(&init_user_ns, inode, mask);
}
...
static int may_create_in_sticky(struct nameidata *nd, struct inode *const inode)
{
	...
	if ((!sysctl_protected_fifos && S_ISFIFO(inode->i_mode)) ||
	    ...
	    uid_eq(current_fsuid(), i_uid_into(user_ns, inode)))
		return 0;
...
{
	if (uid_eq(i_uid_into(user_ns, inode), current_fsuid()))
		return 0;
	if (uid_eq(i_uid_into(user_ns, dir), current_fsuid()))
		return 0;
	return !ns_capable_wrt_inode_uidgid(user_ns, inode, CAP_FOWNER);
}
...
static int may_delete(struct user_namespace *user_ns, struct inode *dir, struct dentry *victim, bool isdir)
{
	...
	error = ns_inode_permission(user_ns, dir, MAY_WRITE | MAY_EXEC);
	if (error)
		return error;
	if (IS_APPEND(dir))
		return -EPERM;
			     ...
			     struct dentry *child)
{
	...
	if (!kuid_has_mapping(s_user_ns, fsuid_into(user_ns)) ||
		...
		return -EOVERFLOW;
	return ns_inode_permission(user_ns, dir, MAY_WRITE | MAY_EXEC);
}
...
static int __vfs_create(struct user_namespace *user_ns, struct inode *dir,
...
int vfs_create(struct inode *dir, struct dentry *dentry, umode_t mode,
		bool want_excl)
{
	return __vfs_create(&init_user_ns, dir, dentry, mode, want_excl);
}
...
struct dentry *__vfs_tmpfile(struct user_namespace *user_ns, struct dentry *dentry,
...
{
	return __vfs_tmpfile(&init_user_ns, dentry, mode, open_flag);
}
		...
		struct file *file)
{
	...
	if (unlikely(error))
		goto out;
	user_ns = mnt_user_ns(path.mnt);
	child = __vfs_tmpfile(user_ns, path.dentry, op->mode, op->open_flag);
	error = PTR_ERR(child);
	if (IS_ERR(child))
		goto out2;
	dput(path.dentry);
...
static int __vfs_mknod(struct user_namespace *user_ns, struct inode *dir,
...
{
	return __vfs_mknod(&init_user_ns, dir, dentry, mode, dev);
}
...
retry:
	...
	switch (mode & S_IFMT) {
		case 0: case S_IFREG:
			error = __vfs_create(user_ns, path.dentry->d_inode,
					     dentry, mode, true);
			if (!error)
				ima_post_path_mknod(dentry);
			break;
		case S_IFCHR: case S_IFBLK:
			error = __vfs_mknod(user_ns, path.dentry->d_inode,
...
static int __vfs_mkdir(struct user_namespace *user_ns, struct inode *dir,
...
{
	return __vfs_mkdir(&init_user_ns, dir, dentry, mode);
}
...
retry:
	...
	if (!error) {
		struct user_namespace *user_ns;
		user_ns = mnt_user_ns(path.mnt);
		error = __vfs_mkdir(user_ns, path.dentry->d_inode, dentry, mode);
	}
	done_path_create(&path, dentry);
	if (retry_estale(error, lookup_flags)) {
		lookup_flags |= LOOKUP_REVAL;
...
static int __vfs_rmdir(struct user_namespace *user_ns, struct inode *dir, struct dentry *dentry)
{
...
{
	return __vfs_rmdir(&init_user_ns, dir, dentry);
}
...
retry:
	...
	if (error)
		goto exit3;
	user_ns = mnt_user_ns(path.mnt);
	error = __vfs_rmdir(user_ns, path.dentry->d_inode, dentry);
exit3:
	dput(dentry);
exit2:
	inode_unlock(path.dentry->d_inode);
...
static int __vfs_unlink(struct user_namespace *user_ns, struct inode *dir,
...
{
	return __vfs_unlink(&init_user_ns, dir, dentry, delegated_inode);
}
...
retry_deleg:
	...
	if (!IS_ERR(dentry)) {
		...
		if (error)
			goto exit2;
		user_ns = mnt_user_ns(path.mnt);
		error = __vfs_unlink(user_ns, path.dentry->d_inode, dentry, &delegated_inode);
exit2:
		dput(dentry);
	}
	inode_unlock(path.dentry->d_inode);
...
static int __vfs_symlink(struct user_namespace *user_ns, struct inode *dir,
...
{
	return __vfs_symlink(&init_user_ns, dir, dentry, oldname);
}
...
retry:
	...
	if (!error) {
		struct user_namespace *user_ns;
		user_ns = mnt_user_ns(path.mnt);
		error = __vfs_symlink(user_ns, path.dentry->d_inode, dentry,
				      from->name);
	}
	done_path_create(&path, dentry);
	if (retry_estale(error, lookup_flags)) {
...
static int __vfs_link(struct dentry *old_dentry, struct user_namespace *user_ns,
...
{
	return __vfs_link(old_dentry, &init_user_ns, dir, new_dentry,
			  delegated_inode);
}
...
retry:
	...
	error = __vfs_link(old_path.dentry, user_ns, new_path.dentry->d_inode,
...
static int __vfs_rename(struct renamedata *rd)
{
	...
	if (new_dir != old_dir) {
		if (is_dir) {
			error = ns_inode_permission(rd->old_user_ns, source, MAY_WRITE);
			if (error)
				return error;
		}
		if ((flags & RENAME_EXCHANGE) && new_is_dir) {
			error = ns_inode_permission(rd->new_user_ns, target, MAY_WRITE);
			if (error)
				return error;
		}
	}
